...
| 阶段2: 历史趋势展示 | 9/12 (47/59子任务) | 2025-11-27 | - |
| 阶段3: 高级筛选功能 | 0/9 | - | - |
| 阶段4: 时间线回放 | 0/11 | - | - |
| 阶段5: 可视化增强 | 0/8 | - | - |
| 阶段6: 数据导出与UX | 0/9 | - | - |
...
- [x] 2.1 创建趋势图表组件
  ...
  - **子任务**:
    - [x] 2.1.1 创建 TrendChart.tsx 文件并导入依赖（React, ReactECharts, dayjs, 历史数据类型）
    - [x] 2.1.2 定义组件 Props 接口（TrendChartProps）
    - [x] 2.1.3 实现组件状态管理（loading, trendData, error）
    - [x] 2.1.4 配置 ECharts 基础选项（title, tooltip, legend, grid, xAxis, yAxis）
    - [x] 2.1.5 实现数据加载逻辑（useEffect 调用 historyAPI.getTrends）
    - [x] 2.1.6 配置图表交互功能（dataZoom, toolbox, tooltip）
    - [x] 2.1.7 添加加载状态和错误处理UI
    - [x] 2.1.8 更新 TODO.md 文档，将2.1标记为完成
...
- [x] 2.2 修改设备详情页面 - 导入部分
  ...
  - **验收**: 编译通过 ✅
  - **子任务**:
    - [x] 2.2.1 导入 TimeRangePicker 组件
    - [x] 2.2.2 导入 TrendChart 组件
    - [x] 2.2.3 导入历史数据类型（TimeRangeQuery, DeviceHistorySnapshot）
    - [x] 2.2.4 验证 TypeScript 编译无错误
    - [x] 2.2.5 更新 TODO.md 文档，将2.2标记为完成
...
- [x] 2.3 修改设备详情页面 - 状态管理
  ...
  - **验收**: 状态初始化正确 ✅
  - **子任务**:
    - [x] 2.3.1 添加 timeRange 状态（useState<TimeRangeQuery>）
    - [x] 2.3.2 添加 playbackSnapshot 状态（useState<DeviceHistorySnapshot | null>）
    - [x] 2.3.3 初始化默认时间范围（最近30天）
    - [x] 2.3.4 更新 TODO.md 文档，将2.3标记为完成

- [x] 2.4 修改设备详情页面 - 添加历史趋势标签页
  - **文件**: `frontend/src/pages/DeviceDetail.tsx`
  - **新增**: "历史趋势" TabPane，包含3个TrendChart
  - **内容**: DGA趋势、温度趋势、老化趋势
  - **验收**: 标签页切换正常，图表显示正确 ✅
  - **子任务**:
    - [x] 2.4.1 在 Tabs 组件添加"历史趋势"标签页
    - [x] 2.4.2 添加 TimeRangePicker 组件（顶部时间选择）
    - [x] 2.4.3 创建 DGA 趋势图（metrics: ["dga.H2", "dga.CH4", "dga.C2H2", "dga.C2H4"]）
    - [x] 2.4.4 创建温度趋势图（metrics: ["thermal.oil_temp", "thermal.hotspot_temp", "thermal.ambient_temp"]）
    - [x] 2.4.5 创建老化趋势图（metrics: ["aging.current_dp", "aging.aging_rate"]）
    - [x] 2.4.6 配置响应式布局（Grid 2列布局）
    - [x] 2.4.7 更新 TODO.md 文档，将2.4标记为完成

- [x] 2.5 修改设备详情页面 - 添加数据回放标签页（预留）
  - **文件**: `frontend/src/pages/DeviceDetail.tsx`
  - **新增**: "数据回放" TabPane（占位符）
  - **验收**: 标签页可切换，显示"功能开发中"提示 ✅
  - **子任务**:
    - [x] 2.5.1 在 Tabs 组件添加"数据回放"标签页
    - [x] 2.5.2 添加 EmptyState 组件显示"功能开发中"
    - [x] 2.5.3 添加提示文本（阶段4开发）
    - [x] 2.5.4 更新 TODO.md 文档，将2.5标记为完成
...
- [x] 2.6 实现历史数据查询方法
  ...
  - **验收**: API返回正确的历史快照数据 ✅（阶段1已验证）
  - **子任务**:
    - [x] 2.6.1 验证 get_device_history() 方法已实现（阶段1已完成）
    - [x] 2.6.2 测试从 timeseries JSON 文件加载数据
    - [x] 2.6.3 测试时间范围过滤功能
    - [x] 2.6.4 测试 granularity 参数（预留，当前不实现降采样）
    - [x] 2.6.5 更新 TODO.md 文档，将2.6标记为完成

- [x] 2.7 实现趋势数据提取方法
  - **文件**: `api/app/services/history_service.py`
  - **方法**: `get_trend_data()`
  - **功能**: 提取指定指标的趋势数据
  - **验收**: 返回TrendDataPoint数组 ✅（阶段1已验证）
  - **子任务**:
    - [x] 2.7.1 验证 get_trend_data() 方法已实现（阶段1已完成）
    - [x] 2.7.2 测试 DGA 指标提取（dga.H2, dga.CH4 等）
    - [x] 2.7.3 测试温度指标提取（thermal.oil_temp 等）
    - [x] 2.7.4 测试老化指标提取（aging.current_dp 等）
    - [x] 2.7.5 验证 _extract_metric_value() 辅助方法正确性
    - [x] 2.7.6 更新 TODO.md 文档，将2.7标记为完成
...
- [x] 2.8 实现批量历史查询方法
  ...
  - **验收**: 返回多设备数据字典 ✅（阶段1已验证）
  - **子任务**:
    - [x] 2.8.1 验证 get_batch_history() 方法已实现（阶段1已完成）
    - [x] 2.8.2 测试多设备并发查询
    - [x] 2.8.3 测试返回数据结构（Dict[device_id, HistoryResponse]）
    - [x] 2.8.4 更新 TODO.md 文档，将2.8标记为完成

- [x] 2.9 实现统计摘要方法
  - **文件**: `api/app/services/history_service.py`
  - **方法**: `get_statistics()`
  - **功能**: 计算均值、最大、最小、标准差、95分位数
  - **验收**: 返回StatisticsSummary数组 ✅（阶段1已验证）
  - **子任务**:
    - [x] 2.9.1 验证 get_statistics() 方法已实现（阶段1已完成）
    - [x] 2.9.2 测试统计计算准确性（avg, min, max, std_dev, percentile_95）
    - [x] 2.9.3 测试多指标统计
    - [x] 2.9.4 更新 TODO.md 文档，将2.9标记为完成
...
- [ ] 2.10 前后端联调 - 历史趋势
  ...
  - **验收**: 数据正确显示，时间范围筛选生效
  - **子任务**:
    - [ ] 2.10.1 启动 Docker 容器（前端 + 后端）
    - [ ] 2.10.2 访问设备详情页，切换到"历史趋势"标签
    - [ ] 2.10.3 测试默认时间范围（最近30天）数据加载
    - [ ] 2.10.4 测试自定义时间范围选择
    - [ ] 2.10.5 测试 DGA/温度/老化 三个图表数据准确性
    - [ ] 2.10.6 测试图表缩放和交互功能
    - [ ] 2.10.7 更新 TODO.md 文档，将2.10标记为完成

- [ ] 2.11 处理边界情况
  - **场景**: 无历史数据、数据不足、API错误
  - **验收**: 显示友好的错误提示或空状态
  - **子任务**:
    - [ ] 2.11.1 处理无历史数据场景（显示 EmptyState）
    - [ ] 2.11.2 处理数据不足场景（<2个数据点，提示"数据不足，无法生成趋势图"）
    - [ ] 2.11.3 处理 API 错误场景（网络错误、500错误，显示错误提示和重试按钮）
    - [ ] 2.11.4 处理设备不存在场景（404错误）
    - [ ] 2.11.5 添加全局错误边界（ErrorBoundary）
    - [ ] 2.11.6 更新 TODO.md 文档，将2.11标记为完成

- [ ] 2.12 优化图表性能
  - **优化**: 大数据集降采样、懒加载
  - **验收**: 1000+数据点渲染流畅（60fps）
  - **子任务**:
    - [ ] 2.12.1 配置 ECharts 性能优化选项（animation: false for large dataset）
    - [ ] 2.12.2 实现数据降采样逻辑（超过1000点时，使用 lttb 算法或按时间聚合）
    - [ ] 2.12.3 配置图表懒加载（Tabs切换时才渲染）
    - [ ] 2.12.4 添加加载骨架屏（LoadingSkeleton）
    - [ ] 2.12.5 优化响应式布局（移动端单列，PC端双列）
    - [ ] 2.12.6 测试大数据集性能（1000+点，监控FPS）
    - [ ] 2.12.7 更新 TODO.md 文档，将2.12标记为完成
